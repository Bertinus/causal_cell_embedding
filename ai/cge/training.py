...
import ai.cge.manager.mlflow.logger as mlflow_logger
import ai.cge.utils.configuration as configuration
import ai.cge.utils.monitoring as monitoring
import copy
import logging
import numpy as np
import os
...
def evaluate_epoch(model, device, data_loader, epoch, exp_name, name='epoch'):
    ...
    model.eval()
    targets, predictions, all_loss, all_losses = [], [], [], []
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(data_loader):
            ...
            # Prediction is the maximal score for each pixel
            # (across output channels).
            targets.append(y.squeeze(1).cpu())

            # TODO: take advantage of auxilary (contextual) outputs?
            predictions.append(torch.argmax(outputs['seg'], dim=1).cpu())

    # Semantic segmentation metrics.
    targets = torch.cat(targets, dim=0)
    predictions = torch.cat(predictions, dim=0)

    # If the network output is smaller than the true output.
    if targets.shape != predictions.shape:

        _, h, w = targets.shape
        predictions = predictions.type(torch.FloatTensor)
        predictions = F.upsample(input=predictions.unsqueeze(1), size=(h, w),
    ...
    score_name, score = 'iou', iou_score
...
def train(cfg, state=None, save_checkpoints=False, save_performance=True):
    ...
    # dataset
    dataset_train = configuration.setup_dataset(cfg, 'train')
    dataset_valid = configuration.setup_dataset(cfg, 'valid')
    dataset_test = configuration.setup_dataset(cfg, 'test')
    ...
    for epoch in range(base_epoch, n_epochs):
        ...
        # Saved every epoch so we can stop at any time...
